...
CREATE TABLE IF NOT EXISTS `withTrip`.`place` (
  ...
  INDEX `fk_board_region1_idx` (`region_region_code` ASC),
...
CREATE TABLE IF NOT EXISTS `withTrip`.`event_box` (
  `eb_num` INT NOT NULL,
  `eb_rank` INT NOT NULL,
  `eb_pull` INT NOT NULL,
  PRIMARY KEY (`eb_num`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `withTrip`.`event_win` (
  `ew_num` INT NOT NULL,
  `member_member_id` VARCHAR(45) NOT NULL,
  `event_event_round` INT NOT NULL,
  `ew_date` DATETIME NOT NULL,
  PRIMARY KEY (`ew_num`),
  INDEX `fk_event_list_member1_idx` (`member_member_id` ASC),
  INDEX `fk_event_win_list_event1_idx` (`event_event_round` ASC),
